...
public class ShootGun : MonoBehaviour {
	...
	void Update () {
		if(Input.GetButtonDown("Fire1") && bulletNum > 0 && Manager.Released && Time.timeScale > 0f){
			...
			Vector3 position = transform.position + new Vector3(0, gameObject.GetComponent<Collider2D>().bounds.size.y * 2, 0);
			var aBullet = Instantiate(bullet, position, Quaternion.identity) as GameObject;
			aBullet.GetComponent<Rigidbody2D>().velocity = new Vector2(0, speed);
			bulletNum --;
